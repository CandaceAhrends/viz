...
const ChartWrapper = () => {
  ...
  useEffect(() => {
    if (selectedStock && liveChart.size) {
      ...
      if (!liveChart.has(symbol)) {
        // fetchStockCandles using the selectedSymbol and date and insert into the liveChart
        fetchStockCandles({ symbol, date }).then((response) => {
          ...
          liveChart.set(symbol, chartCandles);
          setLiveChart(new Map(liveChart));
        });
      } else {
        const stocks = [
